...
namespace Debtor.Common
{
    /// <summary>
    /// NavigationManager 페이지 간의 탐색을 지원합니다.  NavigationManager는
    /// 앞뒤로 이동하는 데 사용되는 명령을 제공하고 앞뒤로 이동하는 데 사용되는 
    /// 표준 마우스 및 키보드 바로 가기를 등록합니다.  또한 SuspensionManger가 통합되어 있어
    /// 페이지 간에 이동할 때 프로세스 수명 관리와 상태 관리를
    /// 처리합니다.
    /// </summary>
    /// <example>
    /// NavigationManager를 사용하려면 다음 두 단계를 따르거나
    /// BasicPage 또는 기타 페이지 항목 템플릿(BlankPage가 아니어야 함)으로 시작하십시오.
    /// 
    /// 1) NaivgationHelper의 인스턴스를 
